...
library(dplyr) # Various data manipulation functions
library(tidyr) # Unite function
library(heatmaply) # Create heatmaps (interactive)
library(RColorBrewer) # Heatmap colors
library(ggridges) # Create density plot
library(scales) # Create density plot, rescale
library(zip) # Zip outputted files
library(readxl) # Read in excel files
library(stringr)

# Shiny specific libraries
library(shiny) # R shiny package
library(DT) # Datatables in Shiny
library(shinybusy) # Busy spinners
library(shinyjs) # Shiny hiding/showing elements, pop-up box
library(shinyvalidate) # Input validation in Shiny
library(shiny.destroy) # Fully remove shiny objects

# Libraries needed for ontology connection
...
PRO_SPARQL <- function(unique_marker_names, NCBI_taxon_ID, onto_endpoint, Match_step) { 
  ...
  "
    ...
    if (length(df) != 0) {
      ...
      df <- df %>% select(-ends_with(c(".type", ".datatype", "lang")))
      ...
      matched_IDs <- distinct(df, PRO_term, PRO_name, Species)
  ...
  #.GlobalEnv$all_ids <- all_ids
  
  # Put all results in one data frame
  # results <- bind_rows(all_ids, .id = "Marker")
...
# Remove unneeded info
species_in_PRO <- species_in_PRO %>% select(-ends_with(c(".type", ".datatype", "lang")))
...
#example_df_input_1 <- read.csv("example_marker_diff_eq_samusik.csv")

# Dataframe of example input data
example_df_input <-
  data.frame(
    "Cluster" = c("1", "2", "3"),
    "Markers" = c(
      "CD4++, CD8-, CCR7-, CD278+, BCL-6+",
      "CD19+, CD1D1+, Ms4a2+",
      "ectonucleotide+, CCR3++, CD123+, apn+, CD16-, PR_000007431++"
    )
  )

# Dataframe of example reference data
example_df_ref <-
  data.frame(
    "Name" = c(
      "Plasma cells",
    ...
    "Markers" = c(
      "CD27+, CD38++, CXCR4+, CD19-, CD20-, IgD-",
      "CD19+, CD20+, CD21+, CD40+, HLA-DR+, CD10-, CD38-, CD23low",
      "CD3+, CD4+, CXCR3+, CCR1+, CCR5+, CD8-, CD19-, CD14-",
      "CCR3+, CCR4+, CCR8+, CD3+, CD4+, CD8-, CD14-, CD19-, CXCR4+",
      "CD117++, CD23+, CD32+, ENPP-3+, CD11b-",
      "CD11b+, CD16+, CD55+, CD44+, CD15+, CD18+"
    )
  )
##################################################################
...
ui <- function(req){
  fluidPage(
    ...
    tabsetPanel(
      ...
      tabPanel("Input expression data",
               br(),
               div(id = "all_sidebar_tab1_step1", sidebarPanel(id = "sidebar_tab1_step1", 
                                                               ...
                                                               fileInput(
                                                                 ...
                                                                 label = HTML(
                                                                   paste(
                                                                     "Upload post-clustering expression <br /> CSV or XLSX file <br />",
                                                                     downloadLink("download_example_expression_cluster", label = "(download example)"),
                                                                     sep = " "
                                                                   )
                                                                 ),
                                                                 multiple = FALSE,
                                                               ...
                                                               textInput(
                                                                 ...
                                                                 label = HTML(
                                                                   paste("Type any marker(s) used for pre-gating, <br /> using commas to seperate <br /> (e.g. CD8+, CD4-)", actionButton("help_pregating", "?"))),
                                                                 value = "",
                                                                 width = NULL
                                                               ),
                                                               ...
                                                               radioButtons(
                                                                 "transform_option",
                                                                 label = HTML(paste("Do you want to arcsinh transform the data?", actionButton("help_transform_option", "?"))),
                                                               ...
                                                               checkboxInput(
                                                                 inputId = "random_1",
                                                                 label = HTML(paste("Randomly downsample expression data", actionButton("help_random_type_1", "?"))),
                                                                 value = FALSE),
                                                               ...
                                                               checkboxInput(
                                                                 inputId = "seed_1",
                                                                 label = HTML(paste("Set seed (to ensure reproducible results)", actionButton("help_seed_type_1", "?"))),
                                                                 value = FALSE),
                                                               ...
                                                               # Submit button
                                                               actionButton("submit_tab1_step1", label = "Submit"),
                                                               
                                                               # Reset button
                                                               actionButton("reset_tab1_step1", label = "Reset")
                                                               
               )),
               
               shinyjs::hidden(div(id = "all_sidebar_tab1_step2", sidebarPanel(id = "sidebar_tab1_step2", 
                                                                               ...
                                                                               radioButtons(
                                                                                 ...
                                                                                 label = HTML(paste("Do you want to use default Median Difference Equation parameters?", actionButton("help_marker_diff_eq_option", "?"))),
                                                                               ...
                                                                               conditionalPanel(
                                                                                 ...
                                                                                 h6(HTML(paste('Marker designated as completely positive', actionButton("help_all_pos", "?")))),
                                                                                 ...
                                                                                 h6(HTML(paste('Marker designated as completely null', actionButton("help_all_null", "?")))),
                                                                               ...
                                                                               # Submit button
                                                                               shinyjs::hidden(actionButton("submit_tab1_step2", label = "Save and continue")),
                                                                               
                                                                               shinyjs::hidden(actionButton("submit_tab1_step2_5", label = "Save and continue")),
                                                                               
                                                                               # Reset button
                                                                               actionButton("reset_tab1_step2", label = "Reset to beginning")
               ))),
               
               shinyjs::hidden(div(id = "all_sidebar_tab1_step3", sidebarPanel(id = "sidebar_tab1_step3", 
                                                                               ...
                                                                               radioButtons(
                                                                                 "reference_type_1",
                                                                                 label = HTML(paste("Reference type", actionButton("help_ref_type", "?"))),
                                                                               ...
                                                                               conditionalPanel(
                                                                                 ...
                                                                                 selectInput(
                                                                                   "species_choice_1",
                                                                                   label = HTML(paste("Select the species", actionButton("help_species", "?"))),
                                                                               ...
                                                                               conditionalPanel(
                                                                                 ...
                                                                                 checkboxGroupInput(
                                                                                   "ontology_type_1",
                                                                                   label = HTML(paste("Select cell type ontology(s)", actionButton("help_ontology_type_1", "?"))),
                                                                                 ...
                                                                                 fileInput(
                                                                                   ...
                                                                                   label = HTML(
                                                                                     paste(
                                                                                       ...
                                                                                       downloadLink("download_example_marker_cell_type_1", label = "(download example)"),
                                                                                       sep = " "
                                                                                     )
                                                                                   ),
                                                                                   multiple = FALSE,
                                                                               ...
                                                                               # Submit button
                                                                               actionButton("submit_tab1_step3", label = "Save and continue"),
                                                                               
                                                                               # Reset button
                                                                               actionButton("reset_tab1_step3", label = "Reset to beginning")
               ))),
               
               shinyjs::hidden(div(id = "all_sidebar_tab1_step4", sidebarPanel(id = "sidebar_tab1_step4", 
                                                                               
                                                                               
                                                                               h5('Cell type matching'),
                                                                               
                                                                               # Submit button
                                                                               actionButton("submit_tab1_step4", label = "Save and continue"),
                                                                               
                                                                               # Reset button
                                                                               actionButton("reset_tab1_step4", label = "Reset to beginning")
                                                                               
               ))),
               
               shinyjs::hidden(div(id = "all_sidebar_tab1_step5", sidebarPanel(id = "sidebar_tab1_step5", 
                                                                               ...
                                                                               # Submit button
                                                                               actionButton("submit_tab1_step5", label = "Save and continue"),
                                                                               
                                                                               # Reset button
                                                                               actionButton("reset_tab1_step5", label = "Reset to beginning")
                                                                               
               ))),
               
               
               shinyjs::hidden(div(id = "all_sidebar_tab1_step6", sidebarPanel(id = "sidebar_tab1_step6", 
                                                                               
                                                                               # Reset button
                                                                               actionButton("reset_tab1_step6", label = "Reset to beginning")
               ...
               mainPanel(align="center",
                         ...
                         # Loading spinner
                         add_busy_spinner(spin = "fading-circle", height = "140px",
                       ...
                       DTOutput("expression_data"),
                         ...
                         conditionalPanel(
                           ...
                           tagList(uiOutput("cutoff_parameters"),
                           
                           # Step 3, dataframe
                           shinyjs::hidden(DTOutput("marker_diff_eq_results_editable")), 
                           
                           uiOutput("marker_diff_eq_heatmap_binary_ex_plot2"),
                         ...
                         shinyjs::hidden(downloadButton("download_marker_diff_eq_csv", label = "Download marker definitions (.csv)", suspendWhenHidden = FALSE)),
                        ...
                        # Step 4, error message if there is a marker duplicated within a cluster
                         uiOutput("ui_marker_sign_error_1"),
                     
                         # Step 4, message unmatched markers in PRO
                         uiOutput("ui_warning_unmatched_markers_1"),
                         
                         # Step 4, dataframe
                     DTOutput("table_new_format_1"),
                     ...
                     DTOutput("table_protein_synonym_1"),
                     ...
                     shinyjs::hidden(downloadButton("download_PRO_GO_matches_1", "Download")),
                     ...
                     tags$div(id = "choose_cluster_1", 
                              uiOutput("specific_cluster_1")),
                 
                     DTOutput("table_cell_types_1"),
                         ...
                         shinyjs::hidden(downloadButton("download_table_cell_types_1", "Download"))
               )
      ...
      tabPanel("Input marker descriptors",
               
               
               div(id = "all_sidebar_tab2_step1", sidebarPanel(id = "sidebar_tab2_step1", 
                                                               ...
                                                               conditionalPanel(
                                                                 ...
                                                                 fileInput(
                                                                   ...
                                                                   label = HTML(paste(
                                                                     ...
                                                                     downloadLink("download_example", label = "(download example)"),
                                                                     sep = " "
                                                                   )),
                                                                   multiple = FALSE,
                                                                   # accept = c('text/csv',
                                                               ...
                                                               textInput(
                                                                 ...
                                                                 label = HTML(
                                                                   paste("Type any marker(s) used for pre-gating, <br /> using commas to seperate <br /> (e.g. CD8+, CD4-)", actionButton("help_pregating_2", "?"))),
                                                                 value = "",
                                                                 width = NULL
                                                               ),
                                                               ...
                                                               radioButtons(
                                                                 "reference_type_2",
                                                                 label = HTML(paste("Reference type", actionButton("help_ref_type_2", "?"))),
                                                               ...
                                                               conditionalPanel(
                                                                 ...
                                                                 selectInput(
                                                                   "species_choice_2",
                                                                   label = HTML(paste("Select the species", actionButton("help_species_2", "?"))),
                                                                   choices = species_in_PRO$full_name,
                                                                   multiple = FALSE
                                                                 )
                                                               ),
                                                               ...
                                                               conditionalPanel(
                                                                 ...
                                                                 checkboxGroupInput(
                                                                   "ontology_type_2",
                                                                   label = HTML(paste("Select cell type ontology(s)", actionButton("help_ontology_type_2", "?"))),
                                                               ...
                                                               conditionalPanel(
                                                                 ...
                                                                 fileInput(
                                                                   ...
                                                                   label = HTML(
                                                                     paste(
                                                                       "Upload reference marker-cell type <br /> CSV or XLSX file <br />",
                                                                       downloadLink("download_example_marker_cell_type_2", label = "(download example)"),
                                                                       sep = " "
                                                                     )
                                                                   ),
                                                                   multiple = FALSE,
                                                                 ...
                                                                 radioButtons(
                                                                   "low_option_upload",
                                                                   label = HTML(paste("Select the interpretation of 'low'", actionButton("help_low_upload", "?"))),
                                                                 ...
                                                                 radioButtons(
                                                                   "high_option_upload",
                                                                   label = HTML(paste("Select the interpretation of 'high'", actionButton("help_high_upload", "?"))),
                                                               ...
                                                               # Submit button
                                                               actionButton("submit_tab2_step1", label = "Save and continue"),
                                                               
                                                               # Reset button
                                                               actionButton("reset_tab2_step1", label = "Reset")
               )),
               
               shinyjs::hidden(div(id = "all_sidebar_tab2_step2", sidebarPanel(id = "sidebar_tab2_step2", 
                                                                               
                                                                               h5('Cell type matching'),
                                                                               
                                                                               # Submit button
                                                                               actionButton("submit_tab2_step2", label = "Save and continue"),
                                                                               
                                                                               # Reset button
                                                                               actionButton("reset_tab2_step2", label = "Reset to beginning")
                                                                               
               ))), 
               
               shinyjs::hidden(div(id = "all_sidebar_tab2_step3", sidebarPanel(id = "sidebar_tab2_step3", 
                                                                               ...
                                                                               radioButtons(
                                                                                 "low_option_2",
                                                                                 label = HTML(paste("Select the interpretation of 'low'", actionButton("help_low_2", "?"))),
                                                                               ...
                                                                               radioButtons(
                                                                                 "high_option_2",
                                                                                 label = HTML(paste("Select the interpretation of 'high'", actionButton("help_high_2", "?"))),
                                                                                 choices = input_choices_high_2,
                                                                                 selected = "high_positive_2"
                                                                               ),
                                                                               
                                                                               # Submit button
                                                                               actionButton("submit_tab2_step3", label = "Save and continue"),
                                                                               
                                                                               # Reset button
                                                                               actionButton("reset_tab2_step3", label = "Reset to beginning")
                                                                               
               ))),
               
               shinyjs::hidden(div(id = "all_sidebar_tab2_step4", sidebarPanel(id = "sidebar_tab2_step4", 
                                                                               
                                                                               
                                                                               h5('Cell type matching'),
                                                                               
                                                                               # Reset button
                                                                               actionButton("reset_tab2_step4", label = "Reset to beginning")
               ...
               mainPanel(align="center",
                         ...
                         # Uploaded reference, error message if the uploaded reference is not a csv, xlsx, or xls file
                         uiOutput("ui_error_check_reference_file_2"), #works
                        
                         # Uploaded reference, error message if the column names for the uploaded reference are incorrect
                         uiOutput("ui_error_column_names_ref_2"), #works
                         
                         # Uploaded reference, error message if there is a marker with no sign in the uploaded reference
                         uiOutput("ui_warning_no_sign_ref_2"), #works
                         
                         # Uploaded reference, error message if there is a marker with no sign in the inputted marker pattern
                         uiOutput("ui_warning_no_sign_2"), #works
                         
                         # Uploaded reference, get statement indicating if/which markers are not in uploaded reference
                         uiOutput("ui_warning_unmatched_markers_uploaded_ref_2"), #works
                         
                         # Uploaded reference,, filter dataframe by cluster
                         tags$div(id = "choose_cluster_uploaded_2", 
                                  uiOutput("specific_cluster_uploaded_2")),
                         
                         #  Uploaded reference, results table
                         DTOutput("uploaded_ref_matches_2_dt"), #works
                         
                         # Uploaded reference, download final dataframe 
                         shinyjs::hidden(downloadButton("download_uploaded_ref_table_cell_types_2", "Download")), #works
                         ...
                         # Step 1, error message if the uploaded input is not a csv, xlsx, or xls file
                         uiOutput("ui_error_check_input_file_2"), #works?
                         
                         # Uploaded reference, error message if the column names for the uploaded query file are incorrect
                         uiOutput("ui_error_column_names_input_2"), #works
                         
                         # Step 1, error message if there is a marker duplicated within a cluster
                         uiOutput("ui_marker_sign_error_2"), #works 
                         ...
                         uiOutput("ui_warning_unmatched_markers_2"),
                         
                         DTOutput("table_new_format_2"),
                         ...
                         # Step 2, dataframe
                         DTOutput("table_protein_synonym_2"),
                         ...
                         shinyjs::hidden(downloadButton("download_PRO_GO_matches_2", "Download")),
                         ...
                         shinyjs::hidden(downloadButton("download_table_cell_types_2", "Download"))
               )
      ) 
    )
  )
...
server <- function(input, output, session) {
  ...
  output$download_example_expression_cluster <- downloadHandler(
    # File name
    filename = function() {
      c("Expression-cluster-Samusik-example.csv")
    },
    # Write to csv
    content = function(file) {
      write.csv(example_df_input_1,  file, row.names = FALSE)
    }
  )

  # Set seed if the user chooses to do so
  global_seed <- eventReactive(input$submit_tab1_step1,{
    req(input$seed_1 == 1)
    input$num_seed_1
  })
  ...
  # Reset everything
  observeEvent(input$reset_tab1_step1, {
    session$reload()
  })

  # Validation rules for text input
  val_cofactor <- InputValidator$new()
  val_cofactor$add_rule("type_cofactor", sv_numeric())
  val_cofactor$enable()

  val_seed <- InputValidator$new()
  val_seed$add_rule("num_seed_1", sv_numeric())
  val_seed$enable()

  val_seed <- InputValidator$new()
  val_seed$add_rule("num_random_1", sv_numeric())
  val_seed$enable()

  # Help buttons for side panel parameters
  observeEvent(input$help_pregating,{
  ...
  observeEvent(input$help_transform_option,{
    showModal(modalDialog(
  ...
  observeEvent(input$help_random_type_1,{
    showModal(modalDialog(
  ...
  observeEvent(input$help_seed_type_1,{
    showModal(modalDialog(
      title = HTML("Set Seed"),
  ...
  df_expr <- reactiveValues(data = NULL)
  
  # Step 1, check if file type of uploaded input file is acceptable
  check_input_file_1 <- eventReactive(input$submit_tab1_step1, {
  ...
  output$ui_error_check_input_file_1 <- renderUI({
    req(input$submit_tab1_step1)
  ...
  uploaded_file <- eventReactive(input$submit_tab1_step1,{
    ...
    file_input <- input$upload_expression_csv
      ...
      if (endsWith(file_input$datapath, ".csv")) {
        ...
        read_file <- read.csv(file_input$datapath, header = TRUE)
  ...
  output$ui_error_column_names_input_1 <- renderUI({
    req(input$submit_tab1_step1)
  ...
  added_MG_markers <- eventReactive(input$submit_tab1_step1,{
  ...
  # Show markers
  uploaded_expression_head <- eventReactive(input$submit_tab1_step1,{
  ...
  observeEvent(input$help_tab1_step1,{
    showModal(modalDialog(
  ...
  observeEvent(input$expression_data_cell_edit, {
    req(input$submit_tab1_step1)
    df_expr$data <<-
      editData(df_expr$data,
  ...
  observeEvent(input$reset_tab1_step2, {
    session$reload()
  })

  # Step 2, help buttons for side panel parameters
  observeEvent(input$help_marker_diff_eq_option,{
    showModal(modalDialog(
  ...
  observeEvent(input$help_pos_neg,{
    showModal(modalDialog(
  ...
  observeEvent(input$help_all_pos,{
    showModal(modalDialog(
  ...
  observeEvent(input$help_all_null,{
    showModal(modalDialog(
  ...
  observeEvent(input$submit_tab1_step2, {

    removeOutput("help_tab1_step1")
    removeOutput("overview_tab1_step1")
    removeOutput("delete_marker")
    removeOutput("expression_data")
    shinyjs::show("overview_tab1_step2")
    removeOutput("submit_tab1_step2")
    ...
    output$cutoff_parameters <- renderUI({
    ...
    output$marker_diff_eq_heatmap_binary_ex_plot = renderPlotly({
      marker_diff_eq_heatmap_binary_ex()
    })
    output$marker_diff_eq_heatmap_raw_ex_plot = renderPlotly({
      marker_diff_eq_heatmap_raw_ex()
    })
    output$expr_heatmap_org_ex_plot = renderPlotly({
      expr_heatmap_org_ex()
    })
    output$expr_density_ex_plot = renderPlot({
      expr_density_ex()
    })
    output$expr_heatmap_max_ex_plot = renderPlotly({
      expr_heatmap_max_ex()
    })
    output$expr_heatmap_min_ex_plot = renderPlotly({
      expr_heatmap_min_ex()
    })
    output$expr_heatmap_std_ex_plot = renderPlotly({
      expr_heatmap_std_ex()
    })
  })

  # Step 2, Median Difference Equation
  raw_marker_diff_eq_results <- eventReactive(input$submit_tab1_step2,{
    ...
    change_names <- match(names(kept_markers), marker_lookup$`Original marker names`)

    names(kept_markers)[na.omit(change_names)] = marker_lookup$`New marker names`[!is.na(change_names)]
    ...
    for (cluster_num in 1:length(each_cluster)) {
      ...
      for (marker in 1:num_markers) {
        ...
        # Get median
        median_cluster_interest <- abs(median(cluster_interest))
        ...
        # Get median
        median_cluster_reference <- abs(median(cluster_reference))
    ...
    df_scaled <- sapply(df_scaled, rescale, to=c(-1,1))
  ...
  heatmap_marker_diff_eq_results <- reactive({
    ...
    z <- names(which(apply(expr, 2, sd) < type_stdr_negative_cutoff))
    ...
    # If the standard deviation is under the a certain value per marker
    z2 <- names(which(apply(expr,2,sd) < type_stdr_positive_cutoff))
  ...
  marker_diff_eq_heatmap_binary_ex <- reactive({
    ...
    binary_heatmap <- heatmaply(marker_diff_eq_results_bi2, scale_fill_gradient_fun = ggplot2::scale_fill_gradient2(low = "#762A83", high = "#1B7837", midpoint = 0, limits = c(-1, 1)),  hide_colorbar = TRUE,
      ...
      layout(height = input$heatmap_height_size, titlefont = list(size=20), margin = list(l=50, r=50, b=100, t=100, pad=4))
  ...
  marker_diff_eq_heatmap_raw_ex <- reactive({
    ...
    color_heat <- colorRampPalette(brewer.pal(n = 9, name = "PRGn"))(length(breaks_list))
    ...
    cont_heatmap <- heatmaply(marker_diff_eq_results2,
  ...
  expr_heatmap_org_ex <- reactive({
    ...
    color_heat <- colorRampPalette(brewer.pal(n = 9, name = "PRGn"))(100)

    expr_heatmap <- heatmaply(expr_median, scale_fill_gradient_fun = ggplot2::scale_fill_gradient2(low = "#762A83", high = "#1B7837", midpoint = 0, limits = c(min(expr_median), max(expr_median))),
  ...
  expr_density <- reactive({
    ...
    ggd <- melt(data.frame(Cluster = cell_clustering, expr_1),
  ...
  expr_density_ex <- reactive({
    ...
    density_plots <- density_plots +
      ...
      facet_wrap( ~ antigen, scales = "free_x", nrow = input$density_facet) +
      theme_ridges() +
      theme(axis.text.x = element_text(size = input$density_row_size), #input$marker_diff_eq_heatmap_raw_row_size),  #17
            axis.text.y = element_text(size = 17), #input$marker_diff_eq_heatmap_raw_col_size), #17
            axis.title.x = element_text(hjust = .5, size = 25),
  ...
  expr_heatmap_max_ex <- reactive({
    ...
    expr_heatmap <- heatmaply(max_med, scale_fill_gradient_fun = ggplot2::scale_fill_gradient2(low = "#762A83", high = "#1B7837", midpoint = 0, limits = c(min(max_med), max(max_med))),
      ...
      layout(height = 425, titlefont = list(size=20), margin = list(l=50, r=50, b=100, t=100, pad=4))
  ...
  expr_heatmap_min_ex <- reactive({
    ...
    expr_heatmap <- heatmaply(min_med, scale_fill_gradient_fun = ggplot2::scale_fill_gradient2(low = "#762A83", high = "#1B7837", midpoint = 0, limits = c(min(min_med), max(min_med))),
  ...
  expr_std <- reactive({
    ...
    sd_med <- apply(expr, 2, sd, na.rm=TRUE)
  ...
  expr_heatmap_std_ex <- reactive({
    ...
    expr_heatmap <- heatmaply(sd_med, scale_fill_gradient_fun = ggplot2::scale_fill_gradient2(low = "#762A83", high = "#1B7837", midpoint = 0, limits = c(min(sd_med), max(sd_med))),
  ...
  output$marker_diff_eq_heatmap_binary_ex_plot2 = renderUI({
 #   req(input$submit_tab1_step2)
    ...
    tagList(
      plotlyOutput("marker_diff_eq_heatmap_binary_ex_plot", width = "100%", height = paste0(heatmap_height)),
            plotlyOutput("marker_diff_eq_heatmap_raw_ex_plot", width = "100%", height = paste0(heatmap_height)),
            plotlyOutput("expr_heatmap_org_ex_plot", width = "100%", height = paste0(heatmap_height)),
            plotOutput("expr_density_ex_plot", width = "100%", height = paste0(density_height)),
            plotlyOutput("expr_heatmap_max_ex_plot"),
            plotlyOutput("expr_heatmap_min_ex_plot"),
            plotlyOutput("expr_heatmap_std_ex_plot"))
  })

  # Step 2, when help button is clicked show message
  observeEvent(input$help_tab1_step2,{
    showModal(modalDialog(
  ...
  # Reset everything
  observeEvent(input$reset_tab1_step3, {
    session$reload()
  })

  # Download example marker-cell type reference data
  output$download_example_marker_cell_type_1 <- downloadHandler(
    # File name
    filename = function() {
      c("Marker-cell-type-example-reference.csv")
    },
    # Write to csv
    content = function(file) {
      write.csv(example_df_ref,  file, row.names = FALSE)
    }
  )

  # Step 3, help buttons for side panel parameters
  observeEvent(input$help_ref_type,{
    showModal(modalDialog(
      ...
      An example marker-cell type reference file is available within the application (click ‘Download Example’ above the file upload box) and on the Github page (within the 'Example Data' folder.)"),
      easyClose = TRUE))
  })

  # Help box for species type
  observeEvent(input$help_species,{
    showModal(modalDialog(
  ...
  observeEvent(input$help_ontology_type_1,{
    showModal(modalDialog(
  ...
  observeEvent(input$submit_tab1_step2_5, {

    df_marker_diff_eq$data <- heatmap_marker_diff_eq_results()

    removeOutput("help_tab1_step2")
    removeOutput("overview_tab1_step2")
    shinyjs::show("overview_tab1_step3")
    shinyjs::show("marker_diff_eq_results_editable")
    removeOutput("marker_diff_eq_heatmap_binary_ex_plot")
    removeOutput("all_sidebar_tab1_step2")
    shinyjs::show("all_sidebar_tab1_step3")
    removeOutput("expr_heatmap_max_ex_plot")
    removeOutput("expr_heatmap_min_ex_plot")
    removeOutput("expr_heatmap_std_ex_plot")

    # Median Difference Equation results
    output$marker_diff_eq_results_editable <- DT::renderDT({
      req(input$submit_tab1_step2_5)
      DT::datatable(
        ...
        formatStyle(names(df_marker_diff_eq$data), color = styleEqual(c("-", "+"), c("white", "white")), fontWeight = 'bold', fontSize = '150%', textAlign = 'center', backgroundColor = styleEqual(c("-", "+"), c("#762A83", "#1B7837")))
     }, server=FALSE)
  ...
  df_marker_diff_eq <- reactiveValues(data = NULL)

  # Step 3, get max, min, std for table download
  max_min_std <- reactive({

  req(input$submit_tab1_step2_5)
#  req(input$download_marker_diff_eq_csv)
  ...
  #  req(input$download_marker_diff_eq_csv)
    ...
    df <- data.frame("Parameter" = c("Date created","Transformation","Cofactor", "Downsample", "Seed", "Positive cutoff","Negative cutoff","Minimum medium all positive cutoff","Maximum medium all positive cutoff","Standard deviation all positive cutoff", "Minimum medium all null cutoff", "Maximum medium all null cutoff", "Standard deviation all null cutoff"),
                     "Value" = c(as.character(Sys.time()), transform_choice, cofactor_num, downsample_num, seed_num, type_positive_cutoff, type_negative_cutoff, type_med_positive_cutoff, type_max_med_positive_cutoff, type_stdr_positive_cutoff, type_med_negative_cutoff, type_max_med_negative_cutoff, type_stdr_negative_cutoff))
    return(df)
  })
 ...
 #   req(input$download_marker_diff_eq_csv)
    ...
    marker_diff_eq_results_string <- marker_diff_eq_results_string %>%
      unite("Marker definition", 2:length(marker_diff_eq_results_string), sep=" ")
  ...
  observeEvent(input$help_tab1_step3,{
    showModal(modalDialog(
      title = HTML("Step 3<br>Override Median Difference Equation Results"),
      HTML("In this step, users can override the results from the Median Difference Equation. The categorical results from Step 2 are shown in an editable data frame. Users can modify the designations for each marker-cluster combination by clicking the box and choosing either positive ('+'), negative ('-'), or null (' ').
      <br>
      <br>
  ...
  # Step 3, edit table
  observeEvent(input$marker_diff_eq_results_cell_edit, {
    req(input$submit_tab1_step2_5)
    df_marker_diff_eq$data <<-
      editData(df_marker_diff_eq$data,
               input$marker_diff_eq_results_cell_edit,
               'marker_diff_eq_results',
               rownames = FALSE)
  })

  # Download marker definition csv files
  output$download_marker_diff_eq_csv <- downloadHandler(
    # Filename
    filename = function() {
    ...
    content = function(file) {
      ...
      for (i in c("a", "b", "c", "d", "e")) {
        ...
        write.csv(to_dl[["data"]][[i]], fileName, row.names = FALSE) # write csv in temp dir
  ...
  # Reset everything
  observeEvent(input$reset_tab1_step4, {
    session$reload()
  })
  ...
  marker_sign_error_1 <- eventReactive(input$submit_tab1_step3, {
  ...
  output$ui_marker_sign_error_1 <- renderUI({
    req(input$submit_tab1_step3)
  ...
  observeEvent(input$help_uploaded_ref_1,{
    showModal(modalDialog(
  ...
  check_reference_file_1 <- eventReactive(input$submit_tab1_step3, {
    req(input$reference_type_1 == 'upload_ref_1')
    req(input$upload_ref_csv_1)
  ...
  output$ui_error_check_reference_file_1 <- renderUI({
    req(input$reference_type_1 == 'upload_ref_1')
    req(input$upload_ref_csv_1)
    req(input$submit_tab1_step3)
  ...
  reformatted_ref_1 <- eventReactive(input$submit_tab1_step3, {
    req(input$reference_type_1 == 'upload_ref_1')
    req(input$upload_ref_csv_1)
    ...
    if (is.null(check_reference_file_1()) == TRUE) {
      ...
      if (endsWith(file_input$datapath, ".csv")) {
        ...
        inputted_df <- read.csv(file_input$datapath, header = TRUE)
  ...
  output$ui_warning_no_sign_ref_1 <- renderUI({
    req(input$reference_type_1 == 'upload_ref_1')
    req(input$upload_ref_csv_1)
    req(input$submit_tab1_step3)
  ...
  output$ui_error_column_names_ref_1 <- renderUI({
    req(input$reference_type_1 == 'upload_ref_1')
    req(input$upload_ref_csv_1)
    req(input$submit_tab1_step3)
  ...
  output$ui_warning_unmatched_markers_uploaded_ref_1 <- renderUI({
    req(input$reference_type_1 == 'upload_ref_1')
    req(input$upload_ref_csv_1)
    req(input$submit_tab1_step3)
      ...
      for (x in 1:length(each_cluster)) {
        ...
        final_matched_output <- ref_df_new %>%
          ...
          dplyr::summarise(
            ...
            `num of inputs that match to cell type` = n()
          )
  ...
  output$specific_cluster_uploaded_1 <- renderUI({
    final_df <- uploaded_ref_results_1()
    if (!is.null(final_df) & length(unique(final_df$Cluster)) > 1) {
      selectInput(inputId = "show_specific_cluster_uploaded_1",
  ...
  observeEvent(input$submit_tab1_step3, {
    req(input$reference_type_1 == 'upload_ref_1')
    req(input$upload_ref_csv_1)
    
    removeOutput("help_tab1_step3")
    removeOutput("spaces1")
    removeOutput("spaces2")
    removeOutput("spaces3")
    removeOutput("overview_tab1_step3")
    removeOutput("marker_diff_eq_heatmap_raw_row_size2")
    removeOutput("marker_diff_eq_heatmap_raw_column_size2")
    removeOutput("heatmap_height_size2")
    removeOutput("density_row_size2")
    removeOutput("density_height_size2")
    removeOutput("density_facet2")
    removeOutput("marker_diff_eq_results_editable")
    removeOutput("marker_diff_eq_heatmap_raw_ex_plot")
    removeOutput("expr_heatmap_org_ex_plot")
    removeOutput("expr_density_ex_plot")
    removeOutput("cutoff_parameters")
    removeOutput("download_marker_diff_eq_csv")
    ...
    if (is.data.frame(reformatted_data_1()) == TRUE &
      ...
      if (!is.null(uploaded_ref_matches_1()) & nrow(uploaded_ref_matches_1()) > 0) {
        ...
        removeOutput("all_sidebar_tab1_step3")
  ...
  output$download_uploaded_ref_table_cell_types_1 <- downloadHandler(
    # File name
    filename = function() {
    ...
    # Write to csv
    content = function(file) {
      write.csv(uploaded_ref_matches_1(),  file, row.names = FALSE)
    }
  )
  ...
  observeEvent(input$help_tab1_step4,{
    showModal(modalDialog(
  ...
  PRO_results_1 <- reactive({
    ...
    SERVICE wikibase:label { bd:serviceParam wikibase:language 'en'. }
      ...
      marker_alt <- read.csv("specific_suggestions_V1.csv", header = TRUE)

      # List of GO complexes that are included in CL definitions
      GO_complexes <- read.csv("GO_complexes_V1.csv", header = TRUE)

      ## List of CD synonym (adapted from list published by HCDM)
      CD_syn <- read.csv("Curated_synonyms_V1.csv", header = TRUE)

      # Auto replacements
      replace_markers <- read.csv("replace_markers_V1.csv", header=TRUE)

      ## Lists of non-specific suggestions
      # List of non-protein words that were found in markers from the deveolpment data
      common_non_protein_words <- read.csv("common_non_protein_words_V1.csv", header = FALSE)

      # List of metal tags that were found in the development data
      commonly_used_metal_tags <- read.csv("commonly_used_metal_tags_V1.csv", header = FALSE)

      # List of fluorophores tags that were  found in the development data
      commonly_used_fluorophores <- read.csv("commonly_used_fluorophores_V1.csv", header = FALSE)
      ...
      if(length(unique_marker_names) != 0) {
        ...
        for (marker in unique_marker_names) {
          ...
          unique_syn_marker_names <- unique(na.omit(matched_row))
...
")
      ...
      final_df <- final_df[order(match(final_df$Match_step, new_order)),]
  ...
  # Step 4, if some markers were not matched to PRO terms, prompt the user to rename them
   return_alt_markers_1 <- eventReactive(input$submit_tab1_step3, {
  ...
  output$ui_warning_unmatched_markers_1 <- renderUI({
    req(input$reference_type_1 == 'internal_ref_1')
    req(input$submit_tab1_step3)
  ...
  observeEvent(input$table_new_format_1_cell_edit, {
    df_1$data <<-
      editData(df_1$data,
  ...
  # Reset everything
  observeEvent(input$reset_tab1_step5, {
    session$reload()
  })
  
  # Step 5, help button for side panel parameters
  observeEvent(input$help_top_N_matches,{
    showModal(modalDialog(
  ...
  observeEvent(input$submit_tab1_step4, {
    removeOutput("table_new_format_1")
    removeOutput("help_tab1_step4")
    removeOutput("ui_warning_unmatched_markers_1")
    removeOutput("overview_tab1_step4") 
    ...
    if (is.data.frame(return_protein_synonym_1()) == TRUE) {
      ...
      removeOutput("all_sidebar_tab1_step4")
  ...
  df2_1 <- reactiveValues(data = NULL)
...
")
  ...
  markers_in_ref <- eventReactive(input$submit_tab1_step4, {
 ...
 #  Merge in info about whether or not marker is in the reference(s)
  return_protein_synonym_1 <- eventReactive(input$submit_tab1_step4, {
    req(input$reference_type_1 == 'internal_ref_1')
  ...
  downloadable_step_1 <- reactive({ # eventReactive(input$submit_tab1_step4, {
    ...
    if (all(remove_link$`PRO/GO Term` == "Did not match")) {
      ...
      remove_link <- remove_link[complete.cases(remove_link), ]
 ...
 # Step 5, 2nd dataset, all info
  ...
  observeEvent(input$help_tab1_step5,{
    showModal(modalDialog(
  ...
  observeEvent(input$delete_PRO_synonym_1, {
  ...
  output$download_PRO_GO_matches_1 <- downloadHandler(
...
######~#~# Step 6 - Match Median Difference Equation results and PRO/GO terms with cell types, show user ranked results and CL definitions #~#~######
  ...
  # Reset everything
  observeEvent(input$reset_tab1_step6, {
    session$reload()
  })
  ...
  observeEvent(input$submit_tab1_step5, {
    removeOutput("help_tab1_step5")
    removeOutput("overview_tab1_step5")
    removeOutput("table_protein_synonym_1")
    removeOutput("delete_PRO_synonym_1")
    removeOutput("download_PRO_GO_matches_1")
    ...
    if (!is.null(return_cell_types_linked_1())) {
      ...
      output$download_table_cell_types_1 <- downloadHandler(
        # File name
        filename = function() {
          paste0(gsub(".csv|.xls|.xlsx", "", input$upload_expression_csv),
        ...
        # Write to csv
        content = function(file) {
          write.csv(return_cell_types_1(),  file, row.names = FALSE)
        }
      )
      removeOutput("all_sidebar_tab1_step5")
      shinyjs::show("all_sidebar_tab1_step6")
  ...
  return_cell_types_1 <- eventReactive(input$submit_tab1_step5, {
  ...
  "
            ...
            if (length(df) != 0) {
              ...
              # Remove unneeded info
              # df <- df %>% select(-ends_with(c(".type", ".datatype")))
              df <- df %>% select(-ends_with(c(".type", ".datatype", "lang")))
  ...
  "
            ...
            if (length(df1) != 0) {
              ...
              # Remove unneeded info
              df1 <- df1 %>% select(-ends_with(c(".type", ".datatype", "lang")))
  ...
  "
            ...
            if (length(df2) != 0) {
              ...
              # Remove unneeded info
              df2 <- df2 %>% select(-ends_with(c(".type", ".datatype", "lang")))
      ...
      final_matched_output <- all_cluster_matches %>%
        ...
        dplyr::summarise(
          ...
          `num of inputs that match to cell type` =  n()) %>%
        dplyr::arrange(Cluster, desc(`num of inputs that match to cell type`))
  ...
  "
        ...
        # Convert from JSON to a list
        df_all_pro_part <- fromJSON(x, flatten = TRUE)

        # Extract the dataframe
        df_all_pro_part <- df_all_pro_part$results$bindings

        # Remove unneeded info
        df_all_pro_part <- df_all_pro_part %>% select(-ends_with(c(".type", ".datatype", "lang")))
  ...
  SERVICE wikibase:label { bd:serviceParam wikibase:language 'en'. }
      ...
      final_matched_output4 <-  tbl_df(final_matched_output4) %>%
        group_by(Cluster) %>%
        filter(as.integer(ordered(-Score)) %in% 1:input$num_top_matches_1)
  ...
  return_cell_types_linked_1 <- eventReactive(input$submit_tab1_step5, {
  ...
  output$specific_cluster_1 <- renderUI({
    final_df <- return_cell_types_linked_1()
    if (!is.null(final_df) & length(unique(final_df$Cluster)) > 1) {
      selectInput(inputId = "show_specific_cluster_1",
                  label = "Filter by cluster:  ",
                  selected = sort(unique(as.character(final_df$Cluster))),
                  choices = sort(unique(as.character(final_df$Cluster))),
                  multiple = TRUE)
  ...
  observeEvent(input$help_tab1_step6,{
    showModal(modalDialog(
  ...
  # Reset everything
  observeEvent(input$reset_tab2_step1, {
    session$reload()
  })

  # Download example marker-cell type reference data
  output$download_example_marker_cell_type_2 <- downloadHandler(
    # File name
    filename = function() {
      c("Marker-cell-type-example-reference.csv")
    },
  ...
  # Help buttons for side panel parameters
  observeEvent(input$help_pregating_2,{
    showModal(modalDialog(
      title = HTML("Type Pre-Gated Markers"),
      HTML("Any markers that were pre-gated should be inputted here. These markers will be excluded from the Median Difference Equation and treated as homogeneous across all clusters.
      <br>
  ...
  observeEvent(input$help_ref_type_2,{
    showModal(modalDialog(
      title = HTML("Select Cell Type Reference"),
      HTML("The user can choose between two options for the descriptive cell type reference:
      <br>
  ...
  # Help box for species type
  observeEvent(input$help_species_2,{
    showModal(modalDialog(
      title = HTML("Specify Species"),
      HTML("Some proteins and cell type names are species-specific within ontologies. Therefore, when using 'Default References', specify the species of your sample(s)."),
      easyClose = TRUE))
  ...
  # Help box for ontology types
  observeEvent(input$help_ontology_type_2,{
    showModal(modalDialog(
      title = HTML("Select Cell Type Ontology(s)"),
      HTML("When using Default References, specify the cell type ontology. The primary source should be the 'Cell Ontology', which contains hundreds of cell types with marker definitions.
      <br>
  ...
  observeEvent(input$help_low_upload,{
    showModal(modalDialog(
  ...
  observeEvent(input$help_high_upload,{
    showModal(modalDialog(
...
# Step 1, uploaded input, error message if input has wrong column names
output$ui_error_column_names_input_2 <- renderUI({
  req(input$input_type == 'csv_upload')
  req(input$submit_tab2_step1)
  ...
  output$ui_warning_no_sign_2 <- renderUI({
    req(input$submit_tab2_step1)
   ...
   added_MG_markers_2 <- eventReactive(input$submit_tab2_step1, {
  ...
  # Step 1, check if file type of uploaded input file is acceptable
  check_input_file_2 <- eventReactive(input$submit_tab2_step1, {
  ...
  # Step 1, outputs error stating that the input file is in the wrong format
  output$ui_error_check_input_file_2 <- renderUI({
    req(input$submit_tab2_step1)
  ...
  marker_sign_error_2 <- eventReactive(input$submit_tab2_step1, {
  ...
  output$ui_marker_sign_error_2 <- renderUI({
    req(input$submit_tab2_step1)
  ...
  observeEvent(input$help_uploaded_ref_2,{
    showModal(modalDialog(
  ...
  check_reference_file_2 <- eventReactive(input$submit_tab2_step1, {
    req(input$reference_type_2 == 'upload_ref_2')
    req(input$upload_ref_csv_2)
  ...
  output$ui_error_check_reference_file_2 <- renderUI({
    req(input$reference_type_2 == 'upload_ref_2')
    req(input$upload_ref_csv_2)
    req(input$submit_tab2_step1)
  ...
  reformatted_ref_2 <- eventReactive(input$submit_tab2_step1, {
    req(input$reference_type_2 == 'upload_ref_2')
    req(input$upload_ref_csv_2)
  ...
  output$ui_warning_no_sign_ref_2 <- renderUI({
    req(input$reference_type_2 == 'upload_ref_2')
    req(input$upload_ref_csv_2)
    req(input$submit_tab2_step1)
  ...
  output$ui_error_column_names_ref_2 <- renderUI({
    req(input$reference_type_2 == 'upload_ref_2')
    req(input$upload_ref_csv_2)
    req(input$submit_tab2_step1)
  ...
  output$ui_warning_unmatched_markers_uploaded_ref_2 <- renderUI({
    req(input$reference_type_2 == 'upload_ref_2')
    req(input$upload_ref_csv_2)
    req(input$submit_tab2_step1)
  ...
  uploaded_ref_matches_2 <- eventReactive(input$submit_tab2_step1, {
    req(input$reference_type_2 == 'upload_ref_2')
    req(input$upload_ref_csv_2)
  ...
  observeEvent(input$submit_tab2_step1, {
    req(input$reference_type_2 == 'upload_ref_2')
    req(input$upload_ref_csv_2)

    removeOutput("app_overview_2")
  ...
  uploaded_ref_results_2 <- eventReactive(input$submit_tab2_step1, {
  ...
  output$uploaded_ref_matches_2_dt <- renderDT({
    req(input$reference_type_2 == 'upload_ref_2')
    req(input$upload_ref_csv_2)
    req(input$submit_tab2_step1)
  ...
  output$specific_cluster_uploaded_2 <- renderUI({
    final_df <- uploaded_ref_results_2()
    if (!is.null(final_df) & length(unique(final_df$Cluster)) > 1) {
      selectInput(inputId = "show_specific_cluster_uploaded_2",
                  label = "Filter by cluster:  ",
                  selected = sort(unique(as.character(final_df$Cluster))),
                  choices = sort(unique(as.character(final_df$Cluster))),
                  multiple = TRUE)
  ...
  output$download_uploaded_ref_table_cell_types_2 <- downloadHandler(
    # File name
    filename = function() {
      if (input$input_type == 'csv_upload') {
        paste0(gsub(".csv|.xls|.xlsx", "", input$csv_upload),
    ...
    # Write to csv
    content = function(file) {
      write.csv(uploaded_ref_matches_2(),  file, row.names = FALSE)
    }
  )
#   #~#~#~#~#~# Internal references #~#~#~#~#~#

  # Step 1, when help button is clicked show message
  observeEvent(input$help_tab2_step1,{
    showModal(modalDialog(
  ...
  observeEvent(input$submit_tab2_step1, {
    ...
    output$table_new_format_2 <- renderDT({
    ...
    # }
    removeOutput("all_sidebar_tab2_step1")
  ...
  return_alt_markers_2 <- eventReactive(input$submit_tab2_step1, {
    if (is.data.frame(reformatted_data_2()) == TRUE) {
...
#   # Step 1, match inputted markers to PRO terms 
  PRO_results_2 <- eventReactive(input$submit_tab2_step1, {
...
")
  ...
  output$ui_warning_unmatched_markers_2 <- renderUI({
    req(input$reference_type_2 == 'internal_ref_2')
    req(input$submit_tab2_step1)
  ...
  # Step 1, option to edit (add new marker names) in the table
  observeEvent(input$table_new_format_2_cell_edit, {
    #  req(input$reference_type_2 == 'internal_ref_2')
    df_1_2$data <<-
      editData(df_1_2$data,
  ...
  # Reset everything
  observeEvent(input$reset_tab2_step2, {
    session$reload()
  })
    
    observeEvent(input$help_low_2,{
      showModal(modalDialog(
        title = HTML("Select the Interpretation of ‘Low’"),
        HTML("Specify how you would like the ‘low’ modifier to be interpreted. This is used to determine how inputted markers will be matched to markers within the reference cell types. Low can be interpreted as either:
      <br> 
    ...
    observeEvent(input$help_high_2,{
      showModal(modalDialog(
        title = HTML("Select the Interpretation of ‘High’"),
        HTML("Specify how you would like the ‘high’ modifier to be interpreted. This is used to determine how inputted markers will be matched to markers within the reference cell types. High can be interpreted as either:
      <br> 
  ...
  df2_2 <- reactiveValues(data = NULL)
  
  # Step 2, when help button is clicked show message
  observeEvent(input$help_tab2_step2,{
    showModal(modalDialog(
  ...
  # Make editable dataframe, show buttons once that dataframe is made
  observeEvent(input$submit_tab2_step2, {
    
    removeOutput("help_tab2_step1")
    removeOutput("ui_warning_unmatched_markers_2")
    removeOutput("overview_tab2_step1") 
    shinyjs::show("overview_tab2_step2")
    removeOutput("table_new_format_2")
    ...
    if (is.data.frame(return_protein_synonym_2()) == TRUE) {
      ...
      removeOutput("all_sidebar_tab2_step2")
  ...
  # Make PRO link clickable
  # Merge in info about whether or not marker is in the reference(s)
  return_protein_synonym_2 <- eventReactive(input$submit_tab2_step2, {
  ...
  new_PRO_matches_2 <- eventReactive(input$submit_tab2_step2, {
...
")
  ...
  observeEvent(input$delete_PRO_synonym_2, {
  ...
  output$download_PRO_GO_matches_2 <- downloadHandler(
  ...
  # Reset everything
  observeEvent(input$reset_tab2_step3, {
    session$reload()
  }) 
  ...
  # Step 3, when help button is clicked show message
  observeEvent(input$help_tab2_step3,{
    showModal(modalDialog(
  ...
  # Get wikidata link for each cell type
  return_cell_types_2 <- eventReactive(input$submit_tab2_step3, {
      ...
      colnames(final_matched_output4)<-
        c(
          ...
          "Score")
            ...
            final_matched_output4 <-  tbl_df(final_matched_output4) %>%
              group_by(Cluster) %>%
              filter(as.integer(ordered(-Score)) %in% 1:input$num_top_matches_2)
  ...
  return_cell_types_linked_2 <- eventReactive(input$submit_tab2_step3, {
  ...
  # Step 3/Uploaded reference, get how many clusters are in final output for filtering purposes
  output$specific_cluster_2 <- renderUI({
    req(input$submit_tab2_step3)
    final_df <- return_cell_types_linked_2()
    if (!is.null(final_df) & length(unique(final_df$Cluster)) > 1) {
      selectInput(inputId = "show_specific_cluster_2",
                  label = "Filter by cluster:  ",
                  selected = sort(unique(as.character(final_df$Cluster))),
                  choices = sort(unique(as.character(final_df$Cluster))),
                  multiple = TRUE)
  ...
  # Step 3/Uploaded reference, if multiple clusters, change dataframe if filtered by cluster
  df_subsettable_2 <- eventReactive(input$submit_tab2_step3, {
  ...
  # Output final dataframe, show buttons once that dataframe is made
  observeEvent(input$submit_tab2_step3, {
    
    removeOutput("help_tab2_step2")
    removeOutput("overview_tab2_step2")
    removeOutput("table_protein_synonym_2")
    removeOutput("delete_PRO_synonym_2")
    removeOutput("download_PRO_GO_matches_2")
    ...
    if (!is.null(return_cell_types_linked_2())) {
      removeOutput("all_sidebar_tab2_step3")
  ...
  output$download_table_cell_types_2 <- downloadHandler(
    # File name
    filename = function() {
      if (input$input_type == 'csv_upload') {
        paste0(gsub(".csv|.xls|.xlsx", "", input$csv_upload),
    ...
    # Write to csv
    content = function(file) {
      write.csv(return_cell_types_2(),  file, row.names = FALSE)
    }
  )
  
  # Reset everything
  observeEvent(input$reset_tab2_step4, {
    session$reload()
  }) 
  }
